...
class PermissionsMovePlacedCardTest {
    ...
    @Test
    fun `A User group user is allowed to move placed card owned by oneself`() =
        runPermissionSuccessTest(processor, MOVE_PLACED_CARD) {
            requestPlacedCard = stub.copy(ownerId = actorOwner)
            setPrincipal(actorOwner, setOf(USER))
        }

    @Test
    fun `A User group user is not allowed to move placed card owned by another user`() =
        runPermissionFailTest(processor, MOVE_PLACED_CARD) {
            requestPlacedCard = stub.copy(ownerId = anyOwner)
            setPrincipal(anyActor, setOf(USER))
        }

    @Test
    fun `An Admin group user is allowed to move placed card owned by any user`() =
        runPermissionSuccessTest(processor, MOVE_PLACED_CARD) {
            requestPlacedCard = stub.copy(ownerId = anyOwner)
