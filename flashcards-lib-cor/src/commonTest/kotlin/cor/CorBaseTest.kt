package  com.github.kondury.flashcards.cor

import com.github.kondury.flashcards.cor.dsl.*
...
class CorBaseTest {
    ...
    fun `worker should not execute when off`() = runTest {
        val worker = CorWorker<TestContext>(
            title = "w1",
            blockActiveIf = { status == CorStatuses.ERROR },
            blockHandle = { history += "w1; " }
        )
        val ctx = TestContext()
        worker.exec(ctx)
    ...
    fun `worker should handle exception`() = runTest {
        val worker = CorWorker<TestContext>(
            ...
            blockOnException = { e -> history += e.message }
        )
        val ctx = TestContext()
        worker.exec(ctx)
    ...
    fun `chain should execute workers`() = runTest {
        val createWorker = { title: String ->
            CorWorker<TestContext>(
                title = title,
                blockActiveIf = { status == CorStatuses.NONE },
                blockHandle = { history += "$title; " }
            )
        }
        val chain = CorChain<TestContext>(
            executors = listOf(createWorker("w1"), createWorker("w2")),
            title = "chain",
        )
        val ctx = TestContext()
        chain.exec(ctx)
    ...
    fun `on should check condition`() = runTest {
        assertEquals("w2; w3; ", execute(rootChain {
            worker {
                activeIf { status == CorStatuses.ERROR }
                handle { history += "w1; " }
            }
            worker {
                activeIf { status == CorStatuses.NONE }
                handle {
                    history += "w2; "
            ...
            worker {
                activeIf { status == CorStatuses.FAILING }
                handle { history += "w3; " }
            }
        }).history)
    }
    ...
    fun `except should execute when exception`() = runTest {
        assertEquals("some error", execute(rootChain {
            worker {
                ...
                onException { history += it.message }
            }
        }).history)
    }
    ...
    fun `complex chain example`() = runTest {
        val chain = rootChain<TestContext> {
            worker {
                ...
                onException { status = CorStatuses.ERROR }
            }

            chain {
                activeIf { status == CorStatuses.RUNNING }
